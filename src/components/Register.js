...
export default function Register() {
    ...
    const handleRegister = async (e) => {
        ...
        const data = {
            "userName": userNameText, "userEmail": userEmailText, "password": passwordText
        }

        try {
            await axios.post('http://localhost:5424/user/register', data)
                .then(response => {
    ...
    return (<Container>
        <Paper elevation={3} style={paperStyle}>
            <Typography variant="h2" className={classes.heading}>Register</Typography>
            <br/>
